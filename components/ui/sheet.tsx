...
const Sheet = React.forwardRef<View, SheetProps>(
  (
    ...
    // Reset initialization state when open prop changes to false
    // This handles cases where navigation interrupts the close animation
    React.useEffect(() => {
      if (!open) {
        hasInitializedOpen.current = false;
      }
    ...
    const getSafeAreaStyle = () => {
      switch (side) {
        ...
        case "top":
          return { paddingTop: insets.top, paddingLeft: insets.left, paddingRight: insets.right };
        case "bottom":
          return { paddingBottom: insets.bottom, paddingLeft: insets.left, paddingRight: insets.right };
        default:
          return { paddingTop: insets.top, paddingBottom: insets.bottom };
      }
    };
